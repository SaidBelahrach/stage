...
export default function Nav(props){
    return <BrowserRouter>
                    <div className="text-light bg-dark "> 
                                ...
                                <Link to={{pathname:"/products",state:"d"}} className="pr-4 h6">Products</Link>
                                <Link to="/cart" className="pr-4 h6">store <span className="badge badge-danger p-0">{props.nb}</span> </Link>
                    </div>
                                <Route path="/" exact component={HomePage}/>
                                <Route path="/products"  render={()=> <ProductsPage products={props.products} ff={props.fct}/> }/>
                                <Route path="/cart" render={ ()=><CartPage ps={props.ps} rm={props.rm}/>}/>
            </BrowserRouter>
}